  ...
  function dropdownSelect() {
    $(".m-form--control--select").each(function (i, select) {
      ...
      options.each(function (j, o) {
        var dropdown__option =
          '<li class="dropdown-select__option  ' +
          ($(o).is(":selected") ? "dropdown-select--selected" : "") +
          ' " data-value="' +
          $(o).val() +
          '" tabindex="0">' +
          $(o).text() +
    ...
    $(document).on("click", ".dropdown-select__option", function () {
      $(this)
        .closest(".dropdown-select__list")
        .find(".dropdown-select--selected")
        .removeClass("dropdown-select--selected");
      $(this).addClass("dropdown-select--selected");
      var text = $(this).text();
      $(this)
        .closest(".dropdown-select")
        .find(".dropdown-select__current")
        .text(text);
    ...
    if (actionsBtnGroup) {
      actionsBtnGroup.forEach(function (actionsBtn) {
        ...
        btnClick.addEventListener("click", function () {
          ...
          const allFormContiners =
            document.querySelectorAll(".m-form--continers");
          allFormContiners.forEach(function (item) {
            if (item.classList.contains("hidden")) {
              item.classList.remove("hidden");
            }
          });
          const allShowInfo = document.querySelectorAll(".show__info");
          allShowInfo.forEach(function (item) {
            if (!item.classList.contains("hidden")) {
              item.classList.add("hidden");
            }
          });
          const iconPencils = document.querySelectorAll(".icon--pencil");
          iconPencils.forEach(function (item) {
            if (!item.classList.contains("hidden")) {
              item.classList.add("hidden");
            }
          });
          const actionEdits = document.querySelectorAll(".actionEdit");
          actionEdits.forEach(function (item) {
            if (item.classList.contains("hidden")) {
              item.classList.remove("hidden");
            }
          });
